...
public class SubContainerBindingFinalizer : ProviderBindingFinalizer
{
	...
	public SubContainerBindingFinalizer(BindInfo bindInfo, object subIdentifier, bool resolveAll, Func<DiContainer, ISubContainerCreator> creatorFactory)
		: base(bindInfo)
	{
	...
	private void FinalizeBindingConcrete(DiContainer container, List<Type> concreteTypes)
	{
		...
		case ScopeTypes.Transient:
			RegisterProvidersForAllContractsPerConcreteType(container, concreteTypes, (DiContainer _, Type concreteType) => new SubContainerDependencyProvider(concreteType, _subIdentifier, _creatorFactory(container), _resolveAll));
			break;
	...
	private void FinalizeBindingSelf(DiContainer container)
	{
		...
		{
		case ScopeTypes.Transient:
			RegisterProviderPerContract(container, (DiContainer _, Type contractType) => new SubContainerDependencyProvider(contractType, _subIdentifier, _creatorFactory(container), _resolveAll));
			break;
		case ScopeTypes.Singleton:
		{
			SubContainerCreatorCached containerCreator = new SubContainerCreatorCached(_creatorFactory(container));
