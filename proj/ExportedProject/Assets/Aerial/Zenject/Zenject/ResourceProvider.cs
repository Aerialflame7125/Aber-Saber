...
public class ResourceProvider : IProvider
{
	...
	public void GetAllInstancesWithInjectSplit(InjectContext context, List<TypeValuePair> args, out Action injectAction, List<object> buffer)
	{
		...
		if (_matchSingle)
		{
			...
			Assert.That(obj != null, "Could not find resource at path '{0}' with type '{1}'", _resourcePath, _resourceType);
			injectAction = null;
			buffer.Add(obj);
		}
		else
		{
			...
			Assert.That(array.Length > 0, "Could not find resource at path '{0}' with type '{1}'", _resourcePath, _resourceType);
			injectAction = null;
			MiscExtensions.AllocFreeAddRange(buffer, array);
		}
	}
}
