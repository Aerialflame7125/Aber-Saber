...
{
    [SerializeField]
    public Saber _leftSaber;

    [SerializeField]
    public Saber _rightSaber;

    public bool sabersAreClashing { get; private set; }
    public Vector3 clashingPoint { get; private set; }

    // NEW PROPERTY: To indicate when both sabers have been assigned by SaberLoader
    public bool IsReady
    {
        get { return _leftSaber != null && _rightSaber != null; }
    }

    private void Update()
    {
        // *** CRITICAL FIX: Check if both sabers are assigned before attempting to use them ***
        if (!IsReady)
        {
            sabersAreClashing = false; // Ensure clashing is false if sabers aren't ready
            return; // Exit Update if sabers are not yet assigned
        }

        Vector3 inbetweenPoint;
        // Now it's safe to access _leftSaber and _rightSaber
    ...
    // Your existing SegmentToSegmentDist method
