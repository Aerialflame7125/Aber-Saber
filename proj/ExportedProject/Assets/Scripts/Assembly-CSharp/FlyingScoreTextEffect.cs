...
public class FlyingScoreTextEffect : FlyingTextEffect
{
	...
	public void InitAndPresent(NoteCutInfo noteCutInfo, Vector3 targetPos, Color color, SaberAfterCutSwingRatingCounter saberAfterCutSwingRatingCounter)
	{
		...
		int beforeCutScore;
		int afterCutScore;
		ScoreController.ScoreWithoutMultiplier(noteCutInfo, saberAfterCutSwingRatingCounter, out beforeCutScore, out afterCutScore);
		InitAndPresent(GetScoreText(beforeCutScore + afterCutScore), _duration, targetPos, color, 2f, false);
	}
	...
	private void HandleSaberAfterCutSwingRatingCounterDidChangeEvent(SaberAfterCutSwingRatingCounter afterCutRating, float rating)
	{
		int beforeCutScore;
		int afterCutScore;
		ScoreController.ScoreWithoutMultiplier(_noteCutInfo, _saberAfterCutSwingRatingCounter, out beforeCutScore, out afterCutScore);
		base.text = GetScoreText(beforeCutScore + afterCutScore);
	}
