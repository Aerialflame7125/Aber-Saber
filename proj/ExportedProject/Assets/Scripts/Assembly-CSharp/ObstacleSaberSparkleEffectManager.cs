...
public class ObstacleSaberSparkleEffectManager : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			Debug.LogError("ObstacleSaberSparkleEffectManager: PlayerController provider returned null!");
			enabled = false;
			return;
		}
		...
		if (_sabers[0] == null || _sabers[1] == null)
		{
			Debug.LogError("ObstacleSaberSparkleEffectManager: One or both sabers are null!");
			enabled = false;
		}
	}
	...
	private void Update()
	{
		...
		foreach (ObstacleController item in activeObstacleControllers)
		{
			...
			{
				Vector3 burnMarkPos;
				if (!_sabers[i].isActiveAndEnabled || !GetBurnMarkPos(bounds, item.transform, _sabers[i].saberBladeBottomPos, _sabers[i].saberBladeTopPos, out burnMarkPos))
				{
					continue;
				}
	...
	private bool GetBurnMarkPos(Bounds bounds, Transform transform, Vector3 bladeBottomPos, Vector3 bladeTopPos, out Vector3 burnMarkPos)
	{
		...
		vector.Normalize();
		float distance;
		if (bounds.IntersectRay(new Ray(bladeBottomPos, vector), out distance) && distance <= num)
		{
