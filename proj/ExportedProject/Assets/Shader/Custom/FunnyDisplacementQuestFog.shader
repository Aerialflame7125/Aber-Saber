Shader "Custom/Urki/ScreenDisplacementQuestFog" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord3.xy = tmp1.yy + tmp1.xz;
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.z = tmp0.z * _Color.w;
                tmp0.z = tmp0.z * 1.01 + -0.01;
				tmp0.xy = tmp0.xy * _DisplacementStrength.xx;
                tmp1.xy = (inp.texcoord3.xy / inp.texcoord3.w);
                tmp0.xy = tmp0.xy * float2(0.001, 0.001);
				tmp1 = BLOOM_FOG_SAMPLE(inp, tmp0.xy);
                tmp2 = BLOOM_FOG_SAMPLE(inp, tmp0.xy);
				tmp1.xyz = (tmp1.x + tmp1.y + tmp1.z) / 3;
				tmp2.xyz = (tmp2.x + tmp2.y + tmp2.z) / 3;
				tmp1.w = 0;
				tmp2.w = 0;
                tmp2 = tmp2 * _Color + (_AddColorIntensity * _Color);
                tmp2 = tmp2 - tmp1;
                //o.sv_target = tmp0.zzzz * tmp2 + tmp1;
				float4 col = tmp0.zzzz * tmp2 + tmp1;
				float3 fogDistance = inp.worldPos + -_WorldSpaceCameraPos; 
  				float4 fogCol = -float4(col.rgb, 1) + BLOOM_FOG_SAMPLE(inp, 0); 
  				fogCol.a = -col.a; 
  				col = col + ((GetFogIntensity(fogDistance, _FogStartOffset, _FogScale) + 1) * fogCol);
				o.sv_target = col;
                return o;
			}
			ENDCG
