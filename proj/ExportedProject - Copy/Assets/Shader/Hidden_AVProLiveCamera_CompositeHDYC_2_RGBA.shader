Shader "Hidden/AVProLiveCamera/CompositeHDYC_2_RGBA" {
	Properties {
	...
	SubShader {
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = tmp0.x ? tmp1.w : tmp1.y;
                tmp1 = tmp1.xxzz - float4(0.5, 0.5, 0.5, 0.5);
                tmp0.x = tmp0.x - 0.0627451;
                tmp0.yzw = tmp1.xyw * float3(1.793, 0.534, 2.115);
                tmp0.z = tmp0.x * 1.164 + -tmp0.z;
                tmp0.xy = saturate(tmp0.xx * float2(1.164, 1.164) + tmp0.yw);
                tmp2.xz = log(tmp0.xy);
                tmp0.x = saturate(-tmp1.z * 0.213 + tmp0.z);
                tmp2.y = log(tmp0.x);
                tmp0.xyz = tmp2.xyz * float3(2.2, 2.2, 2.2);
