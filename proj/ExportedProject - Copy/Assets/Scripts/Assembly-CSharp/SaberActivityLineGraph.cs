...
public class SaberActivityLineGraph : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			_pointPositions[i] = new Vector3((float)i * _pointDistance, 0f, 0f);
	...
	private IEnumerator UpdateGraphCoroutine()
	{
		...
		while (true)
		{
			...
			foreach (float pointsValue in _pointsValues)
			{
				...
				_pointPositions[pointIndex] = new Vector3((float)pointIndex * _pointDistance, num * _scale);
				pointIndex++;
			}
			_lineRenderer.SetPositions(_pointPositions);
			...
			{
				float num2 = item;
				_pointPositions[pointIndex] = new Vector3((float)pointIndex * _pointDistance, num2 * _scale);
				pointIndex++;
			}
			_lineRenderer2.SetPositions(_pointPositions);
