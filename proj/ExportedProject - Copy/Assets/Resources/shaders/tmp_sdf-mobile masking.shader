	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp2;
                float4 tmp3;
                tmp0.x = _MaskInverse ? 1.0 : 0.0;
                tmp1 = tex2D(_MaskTex, inp.texcoord.zw);
                tmp0.x = tmp0.x - tmp1.w;
                tmp0.y = 1.0 - _MaskWipeControl;
                tmp0.x = tmp0.y * _MaskEdgeSoftness + abs(tmp0.x);
                tmp0.x = tmp0.x - _MaskWipeControl;
                tmp0.x = saturate(tmp0.x / _MaskEdgeSoftness);
                ...
                tmp3.xyz = tmp2.www * _MaskEdgeColor.xyz;
                tmp0.yzw = tmp1.xyz * tmp0.yyy + -tmp3.xyz;
                tmp2.xyz = tmp0.xxx * tmp0.yzw + tmp3.xyz;
