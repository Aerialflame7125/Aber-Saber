...
public class SteamVR_RenderModel : MonoBehaviour
{
	...
	private IEnumerator SetModelAsync(string renderModelName)
	{
		...
		using (RenderModelInterfaceHolder holder = new RenderModelInterfaceHolder())
		{
			...
			if (count != 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (renderModels.GetComponentRenderModelName(renderModelName, stringBuilder.ToString(), stringBuilder2, componentName) != 0)
					{
						...
						RenderModel renderModel = models[text] as RenderModel;
			...
			{
				RenderModel renderModel2 = models[renderModelName] as RenderModel;
				renderModelNames = ((renderModel2 != null && !(renderModel2.mesh == null)) ? new string[0] : new string[1] { renderModelName });
			}
			while (true)
			{
				bool loading = false;
				...
				if (material == null || material.mainTexture == null)
				{
					...
					while (true)
					{
						...
						case EVRRenderModelError.None:
						{
							...
							Texture2D texture2D = new Texture2D(renderModel_TextureMap_t.unWidth, renderModel_TextureMap_t.unHeight, TextureFormat.RGBA32, false);
	...
	public void UpdateComponents(CVRRenderModels renderModels)
	{
		...
		for (int i = 0; i < transform.childCount; i++)
		{
			Transform child = transform.GetChild(i);
			string value;
			if (!nameCache.TryGetValue(child.GetInstanceID(), out value))
			{
				value = child.name;
