...
internal static class SqliteConnectionPool
{
	...
	internal static void ClearAllPools()
	{
		lock (_connections)
		{
			foreach (KeyValuePair<string, Pool> connection in _connections)
			{
				...
				{
					if (connection.Value.Queue.Dequeue().Target is SqliteConnectionHandle sqliteConnectionHandle)
					{
	...
	internal static void ClearPool(string fileName)
	{
		lock (_connections)
		{
			...
			while (value.Queue.Count > 0)
			{
				if (value.Queue.Dequeue().Target is SqliteConnectionHandle sqliteConnectionHandle)
				{
					sqliteConnectionHandle.Dispose();
				}
			}
