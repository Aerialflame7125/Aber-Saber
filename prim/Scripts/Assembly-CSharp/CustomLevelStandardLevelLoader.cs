...
public class CustomLevelStandardLevelLoader : ScriptableObject
{
	...
	private IEnumerator LoadStandardLevelCoroutine(CustomLevelInfoWrapper customLevelInfoWrapper, Action<StandardLevelSO> callback)
	{
		...
		string songName = customLevelInfoWrapper.customLevelInfo.songName;
		string songSubName = customLevelInfoWrapper.customLevelInfo.songSubName;
		string authorName = customLevelInfoWrapper.customLevelInfo.authorName;
		float beatsPerMinute = customLevelInfoWrapper.customLevelInfo.beatsPerMinute;
		float previewStartTime = customLevelInfoWrapper.customLevelInfo.previewStartTime;
		float previewDuration = customLevelInfoWrapper.customLevelInfo.previewDuration;
		...
		yield return _customLevelAudioClipLoader.LoadAudioClipFromPathCoroutine(customLevelInfoWrapper.audioPath, loadAudioClip, stream: true);
		Sprite coverImage = null;
		SceneInfo environmentSceneInfo = LoadSceneInfo(customLevelInfoWrapper.customLevelInfo.environmentName);
		StandardLevelSO.DifficultyBeatmap[] difficultyLevels = null;
		BeatmapDataSO[] beatmapsData = new BeatmapDataSO[customLevelInfoWrapper.customLevelInfo.difficultyLevels.Length];
		...
		{
			difficultyLevels = LoadDifficultyLevels(customLevelInfoWrapper.customLevelInfo.difficultyLevels, customLevelInfoWrapper.path, beatmapsData);
		};
	...
	{
		if (!difficultyLevelsInfo.difficulty.LevelDifficultyFromName(out var levelDifficulty))
		{
			return null;
		}
		int difficultyRank = difficultyLevelsInfo.difficultyRank;
		float noteJumpMovementSpeed = difficultyLevelsInfo.noteJumpMovementSpeed;
		string jsonData = File.ReadAllText(Path.Combine(basePath, difficultyLevelsInfo.jsonPath));
