...
public class XWeaponTrail : MonoBehaviour
{
	...
	private List<Element> _snapshotList = new List<Element>();

	private ElementPool _elemPool;
	...
	public Vector3 CurHeadPos => (_pointStart.position + _pointEnd.position) / 2f;

	protected virtual Color Color => _color;
	...
	private void UpdateVertex()
	{
		...
		for (int i = 0; i < _granularity; i++)
		{
			...
			ref Color reference = ref pool.Colors[num];
			reference = Color;
			zero.x = 0f;
			zero.y = num2;
			pool.UVs[num] = zero;
			pool.Vertices[num + 1] = vector;
			ref Color reference2 = ref pool.Colors[num + 1];
			reference2 = Color;
			zero.x = 0.5f;
			zero.y = num2;
			pool.UVs[num + 1] = zero;
			pool.Vertices[num + 2] = vector4;
			ref Color reference3 = ref pool.Colors[num + 2];
			reference3 = Color;
			zero.x = 1f;
			zero.y = num2;
			pool.UVs[num + 2] = zero;
