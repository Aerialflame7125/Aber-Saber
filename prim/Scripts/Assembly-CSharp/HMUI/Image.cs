...
public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
{
	...
	private void GenerateSlicedSprite(VertexHelper toFill)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			ref Vector2 reference3 = ref s_UV2Scratch[i];
			reference3 = s_VertScratch[i];
		...
		ref Vector2 reference4 = ref s_UVScratch[0];
		reference4 = new Vector2(vector.x, vector.y);
		ref Vector2 reference5 = ref s_UVScratch[1];
		reference5 = new Vector2(vector2.x, vector2.y);
		ref Vector2 reference6 = ref s_UVScratch[2];
		reference6 = new Vector2(vector2.z, vector2.w);
		ref Vector2 reference7 = ref s_UVScratch[3];
		reference7 = new Vector2(vector.z, vector.w);
		toFill.Clear();
		for (int k = 0; k < 3; k++)
		{
			int num = k + 1;
