...
public class SegmentedControl : MonoBehaviour
{
	...
	private void CreateCells()
	{
		...
		for (int i = 0; i < _numberOfColumns; i++)
		{
			...
			segmentedControlCell.gameObject.SetActive(value: true);
			segmentedControlCell.SegmentedControlSetup(this, i);
			segmentedControlCell.ChangeSelection(_selectedColumn == i, SegmentedControlCell.TransitionType.Instant, useCallback: false, ignoreCurrentValue: true);
			segmentedControlCell.ChangeHighlight(value: false, SegmentedControlCell.TransitionType.Instant, ignoreCurrentValue: true);
			...
			{
				segmentedControlCell.transform.SetParent(base.transform, worldPositionStays: false);
			}
	...
	{
		_cells[_selectedColumn].ChangeSelection(value: false, SegmentedControlCell.TransitionType.Instant, useCallback: false, ignoreCurrentValue: false);
	...
	public void SelectColumn(int selectedColumn)
	{
		...
		for (int i = 0; i < _numberOfColumns; i++)
		{
			_cells[i].ChangeSelection(i == selectedColumn, SegmentedControlCell.TransitionType.Instant, useCallback: false, ignoreCurrentValue: false);
		}
	}
}
