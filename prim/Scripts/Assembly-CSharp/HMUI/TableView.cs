...
public class TableView : MonoBehaviour
{
	...
	private void Init()
	{
		...
		if (!_contentTransform)
		{
			...
			{
				gameObject.transform.SetParent(_scrollRect.viewport, worldPositionStays: false);
			}
	...
	private void RefreshScrollButtons()
	{
		...
		if ((bool)_pageDownButton)
		{
			...
			if (num < 0.01f)
			{
				_pageDownButton.gameObject.SetActive(value: false);
			}
			else
			{
				_pageDownButton.gameObject.SetActive(value: true);
			}
		}
		if ((bool)_pageUpButton)
		{
			...
			if (num < 0.01f)
			{
				_pageUpButton.gameObject.SetActive(value: false);
			}
			else
			{
				_pageUpButton.gameObject.SetActive(value: true);
			}
		}
	}
	...
	private void RefreshCells(bool forced)
	{
		...
		for (int num5 = _visibleCells.Count - 1; num5 >= 0; num5--)
		{
			...
			{
				tableCell.gameObject.SetActive(value: false);
		...
		for (int i = num3; i <= num4; i++)
		{
			...
			if (!(tableCell2 != null) || forced)
			{
				...
				tableCell2.gameObject.SetActive(value: true);
				tableCell2.TableViewSetup(this, i);
				tableCell2.ChangeSelection(_selectedRows.Contains(i), TableCell.TransitionType.Instant, callbackTable: false, ignoreCurrentValue: true);
				tableCell2.ChangeHighlight(value: false, TableCell.TransitionType.Instant, ignoreCurrentValue: true);
				if (tableCell2.transform.parent != _contentTransform)
				{
					tableCell2.transform.SetParent(_contentTransform, worldPositionStays: false);
				}
	...
	public void CellSelectionStateDidChange(TableCell changedCell)
	{
		...
		if (selectionType != SelectionType.Multiple)
		{
			foreach (TableCell visibleCell in _visibleCells)
			{
				...
				{
					visibleCell.ChangeSelection(value: false, TableCell.TransitionType.Instant, callbackTable: false, ignoreCurrentValue: false);
				}
			}
		}
	...
	public void SelectRow(int row, bool callbackTable = false)
	{
		foreach (TableCell visibleCell in _visibleCells)
		{
			visibleCell.ChangeSelection(value: false, TableCell.TransitionType.Instant, callbackTable: false, ignoreCurrentValue: false);
		...
		RefreshCells(forced: true);
		if (callbackTable && this.didSelectRowEvent != null)
		{
	...
	{
		float height = _scrollRectTransform.rect.height;
		float num = Mathf.Max(0f, (float)_numberOfRows * _rowHeight - height);
		int num2 = Mathf.FloorToInt(height / _rowHeight);
		float num3 = (float)(row + 1 + num2 / 2) * _rowHeight;
		float targetVerticalNormalizedPosition = 0f;
		if (num > Mathf.Epsilon)
		{
			targetVerticalNormalizedPosition = Mathf.Max(0f, 1f - Mathf.Max(0f, num3 - height) / num);
		}
		_targetVerticalNormalizedPosition = targetVerticalNormalizedPosition;
		if (!animated)
		{
