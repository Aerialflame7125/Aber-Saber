...
public class VRGraphicRaycaster : BaseRaycaster
{
	private struct TGraphicRaycastResult
	{
		...
		{
			return $"[{graphic.gameObject.name} - {distance}, {pointerPosition}]";
		}
	}
	...
	private Canvas m_canvas => __canvas ?? (__canvas = GetComponent<Canvas>());

	public override Camera eventCamera => __eventCamera ?? (__eventCamera = ((!(m_canvas.worldCamera != null)) ? Camera.main : m_canvas.worldCamera));
	...
	public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
	{
		...
		if (Physics.Raycast(ray, out var hitInfo, eventData.pointerCurrentRaycast.depth, m_BlockingMask))
		{
