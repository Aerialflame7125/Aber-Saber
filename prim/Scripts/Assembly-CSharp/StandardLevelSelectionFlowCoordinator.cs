...
public class StandardLevelSelectionFlowCoordinator : FlowCoordinator
{
	[SerializeField]
	...
	public void Present(VRUIViewController parentViewController, IStandardLevel[] levels, GameplayMode gameplayMode)
	{
		...
		_levelSelectionNavigationController.InitWithGameplayModeIndicator(gameplayMode, showBackButton: true);
		...
		_levelListViewController.Init(levels, showHowToPlayViewController: false);
	...
	private void HandleLevelListViewControllerDidSelectLevel(StandardLevelListViewController viewController, IStandardLevel level)
	{
		...
		{
			_levelDifficultyViewController.Init(level.difficultyBeatmaps, showHowToPlayViewController: false);
