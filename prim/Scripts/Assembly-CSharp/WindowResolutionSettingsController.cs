...
public class WindowResolutionSettingsController : ListSettingsController
{
	...
	protected override void GetInitValues(out int idx, out int numberOfElements)
	{
		...
		for (int i = 0; i < Screen.resolutions.Length; i++)
		{
			...
			{
				ref Vector2Int reference = ref _windowResolutions[numberOfElements];
				reference = new Vector2Int(width, height);
		...
		{
			idx = numberOfElements;
			ref Vector2Int reference2 = ref _windowResolutions[idx];
			reference2 = _mainSettingsModel.windowResolution;
			numberOfElements++;
		}
	}
