...
public class Uri : ISerializable
{
	...
	public unsafe override bool Equals(object comparand)
	{
		...
		if (NotAny(Flags.AllUriInfoSet) || result.NotAny(Flags.AllUriInfoSet))
		{
			if (!IsUncOrDosPath)
			{
				...
				{
					fixed (char* @string = m_String)
					{
						fixed (char* string2 = result.m_String)
						{
		...
		if (!UserDrivenParsing && !result.UserDrivenParsing && Syntax.IsSimple && result.Syntax.IsSimple)
		{
			if (InFact(Flags.CanonicalDnsHost) && result.InFact(Flags.CanonicalDnsHost))
			{
				...
				string string3 = result.m_String;
	...
	private unsafe void ParseRemaining()
	{
		...
		if (!UserDrivenParsing)
		{
			...
			fixed (char* @string = m_String)
			{
				if (num > scheme && IsLWS(@string[num - 1]))
				{
					num--;
					while (num != scheme && IsLWS(@string[(int)(--num)]))
					{
					}
					num++;
				}
				...
				else
				{
					...
					while (num2 < num3)
					{
						if (m_Syntax.SchemeName[num2] != @string[scheme + num2])
						{
							flags |= Flags.SchemeNotCanonical;
						}
						num2++;
					}
					if ((m_Flags & Flags.AuthorityFound) != Flags.Zero && (scheme + num2 + 3 >= num || @string[scheme + num2 + 1] != '/' || @string[scheme + num2 + 2] != '/'))
					{
						flags |= Flags.SchemeNotCanonical;
					}
				}
				if ((m_Flags & Flags.HasUserInfo) != Flags.Zero)
				{
					...
					check = CheckCanonical(@string, ref scheme, m_Info.Offset.Host, '@');
			...
			fixed (char* string2 = m_String)
			{
				check = ((!IsImplicitFile && (flags2 & (UriSyntaxFlags.MayHaveQuery | UriSyntaxFlags.MayHaveFragment)) != 0) ? CheckCanonical(string2, ref scheme, num, ((flags2 & UriSyntaxFlags.MayHaveQuery) != 0) ? '?' : (m_Syntax.InFact(UriSyntaxFlags.MayHaveFragment) ? '#' : '\ufffe')) : CheckCanonical(string2, ref scheme, num, '\uffff'));
				if ((m_Flags & Flags.AuthorityFound) != Flags.Zero && (flags2 & UriSyntaxFlags.PathIsRooted) != 0 && (m_Info.Offset.Path == num || (string2[(int)m_Info.Offset.Path] != '/' && string2[(int)m_Info.Offset.Path] != '\\')))
				{
					flags |= Flags.FirstSlashAbsent;
				}
			}
			bool flag2 = false;
			if (IsDosPath || ((m_Flags & Flags.AuthorityFound) != Flags.Zero && ((flags2 & (UriSyntaxFlags.ConvertPathSlashes | UriSyntaxFlags.CompressPath)) != 0 || m_Syntax.InFact(UriSyntaxFlags.UnEscapeDotsAndSlashes))))
			{
				if ((check & Check.DotSlashEscaped) != 0 && m_Syntax.InFact(UriSyntaxFlags.UnEscapeDotsAndSlashes))
				{
			...
			if ((check & Check.DisplayCanonical) == 0 && ((m_Flags & Flags.ImplicitFile) == Flags.Zero || (m_Flags & Flags.UserEscaped) != Flags.Zero || (check & Check.ReservedFound) != 0))
			{
				flags |= Flags.PathNotCanonical;
				flag2 = true;
			}
			if ((m_Flags & Flags.ImplicitFile) != Flags.Zero && (check & (Check.EscapedCanonical | Check.ReservedFound)) != 0)
			{
				check &= ~Check.EscapedCanonical;
			}
			...
			fixed (char* string3 = m_String)
			{
				if (scheme < num && string3[(int)scheme] == '?')
				{
					scheme++;
					check = CheckCanonical(string3, ref scheme, num, ((flags2 & UriSyntaxFlags.MayHaveFragment) != 0) ? '#' : '\ufffe');
					if ((check & Check.DisplayCanonical) == 0)
					{
			...
			fixed (char* string4 = m_String)
			{
				if (scheme < num && string4[(int)scheme] == '#')
				{
					scheme++;
					check = CheckCanonical(string4, ref scheme, num, '\ufffe');
					if ((check & Check.DisplayCanonical) == 0)
					{
	...
	private unsafe ushort CheckAuthorityHelper(char* pString, ushort idx, ushort length, ref ParsingError err, ref Flags flags, UriParser syntax, ref string newHost)
	{
		...
		else if ((flags2 & UriSyntaxFlags.AllowDnsHost) != 0 && ((syntax.InFact(UriSyntaxFlags.AllowIriParsing) && flag3) || syntax.InFact(UriSyntaxFlags.AllowIdn)) && DomainNameHelper.IsValidByIri(pString, num2, ref i, ref notCanonical, StaticNotAny(flags, Flags.ImplicitFile)))
		{
		...
		else if ((flags2 & UriSyntaxFlags.AllowUncHost) != 0 && UncNameHelper.IsValid(pString, num2, ref i, StaticNotAny(flags, Flags.ImplicitFile)) && i - num2 <= 256)
		{
	...
	private void InitializeUri(ParsingError err, UriKind uriKind, out UriFormatException e)
	{
		...
		{
			if (IsImplicitFile && (uriKind != 0 || m_String.Length <= 0 || m_String[0] != '/' || useDotNetRelativeOrAbsolute))
			{
		...
		{
			if (err != 0 && uriKind != UriKind.Absolute && err <= ParsingError.EmptyUriString)
			{
				e = null;
	...
	internal unsafe bool InternalIsWellFormedOriginalString()
	{
		...
		fixed (char* @string = m_String)
		{
			...
			if (!IsAbsoluteUri)
			{
				...
				return (CheckCanonical(@string, ref idx, (ushort)m_String.Length, '\ufffe') & (Check.EscapedCanonical | Check.BackslashInPath)) == Check.EscapedCanonical;
			}
			if (IsImplicitFile)
			{
				return false;
			...
			if ((m_Flags & Flags.CanonicalDnsHost) == Flags.Zero && HostType != Flags.IPv6HostType)
			{
				...
				Check check = CheckCanonical(@string, ref idx, m_Info.Offset.Path, '/');
			...
			if ((m_Flags & (Flags.SchemeNotCanonical | Flags.AuthorityFound)) == (Flags.SchemeNotCanonical | Flags.AuthorityFound))
			{
				...
				while (@string[(int)idx++] != ':')
				{
				}
				if (idx + 1 >= m_String.Length || @string[(int)idx] != '/' || @string[idx + 1] != '/')
				{
					return false;
				}
			}
