...
public class CultureInfoConverter : TypeConverter
{
	private class CultureComparer : IComparer
	{
		...
		public int Compare(object item1, object item2)
		{
			...
			string cultureName = converter.GetCultureName((CultureInfo)item1);
			string cultureName2 = converter.GetCultureName((CultureInfo)item2);
	...
	private static class CultureInfoMapper
	{
		...
		{
			cultureInfoNameMap = new Dictionary<string, string>
			{
				{ "Afrikaans", "af" },
	...
	private string DefaultCultureString => global::SR.GetString("(Default)");
	...
	public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
	{
		if (value is string)
		{
			...
			try
			{
				...
				if (cultureInfo == null)
				{
					IEnumerator enumerator = ((IEnumerable)GetStandardValues(context)).GetEnumerator();
					while (enumerator.MoveNext())
					{
						CultureInfo cultureInfo2 = (CultureInfo)enumerator.Current;
						if (cultureInfo2 != null && string.Compare(GetCultureName(cultureInfo2), text, StringComparison.Ordinal) == 0)
						{
				...
				if (cultureInfo == null)
				{
					...
					while (enumerator2.MoveNext())
					{
						...
						if (cultureInfo3 != null && GetCultureName(cultureInfo3).ToLower(CultureInfo.CurrentCulture).StartsWith(text))
						{
			...
			if (cultureInfo == null)
			{
				throw new ArgumentException(global::SR.GetString("The {0} culture cannot be converted to a CultureInfo object on this computer.", (string)value));
			}
			return cultureInfo;
		}
