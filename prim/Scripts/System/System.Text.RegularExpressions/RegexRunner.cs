...
public abstract class RegexRunner
{
	...
	protected internal Match Scan(Regex regex, string text, int textbeg, int textend, int textstart, int prevlen, bool quick, TimeSpan timeout)
	{
		...
		ignoreTimeout = Regex.InfiniteMatchTimeout == timeout;
		this.timeout = (ignoreTimeout ? ((int)Regex.InfiniteMatchTimeout.TotalMilliseconds) : ((int)(timeout.TotalMilliseconds + 0.5)));
		runregex = regex;
		runtext = text;
		runtextbeg = textbeg;
		runtextend = textend;
	...
	{
		if (!ignoreTimeout && --timeoutChecksToSkip == 0)
		{
			timeoutChecksToSkip = 1000;
			DoCheckTimeout();
		}
	...
	private void DoCheckTimeout()
	{
		...
		if (tickCount < timeoutOccursAt || (0 > timeoutOccursAt && 0 < tickCount))
		{
			return;
		}
