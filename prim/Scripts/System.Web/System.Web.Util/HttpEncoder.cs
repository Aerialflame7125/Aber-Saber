...
public class HttpEncoder
{
	...
	///         <paramref name="offset" /> is less than zero or greater than the length of the <paramref name="bytes" /> array. -or-
	...
	{
		string encoderType = HttpRuntime.Section.EncoderType;
		if (string.Compare(encoderType, "System.Web.Util.HttpEncoder", StringComparison.OrdinalIgnoreCase) == 0)
		{
			return Default;
		}
		Type type = Type.GetType(encoderType, throwOnError: false);
		if (type == null)
		{
			throw new ConfigurationErrorsException($"Could not load type '{encoderType}'.");
		}
		if (!typeof(HttpEncoder).IsAssignableFrom(type))
		{
			throw new ConfigurationErrorsException($"'{encoderType}' is not allowed here because it does not extend class 'System.Web.Util.HttpEncoder'.");
		}
		return Activator.CreateInstance(type, nonPublic: false) as HttpEncoder;
	}
