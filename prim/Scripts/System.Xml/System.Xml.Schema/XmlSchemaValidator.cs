...
public sealed class XmlSchemaValidator
{
	...
	public void Initialize()
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("The transition from the '{0}' method to the '{1}' method is not allowed.", MethodNames[(int)currentState], MethodNames[1]));
		}
	...
	private void CheckStateTransition(ValidatorState toState, string methodName)
	{
		if (!ValidStates[(int)currentState, (int)toState])
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("It is invalid to call the '{0}' method in the current state of the validator. The '{1}' method must be called before proceeding with validation.", methodName, MethodNames[1]));
			}
			throw new InvalidOperationException(Res.GetString("The transition from the '{0}' method to the '{1}' method is not allowed.", MethodNames[(int)currentState], methodName));
		}
	...
	internal static string PrintExpectedElements(ArrayList expected, bool getParticles)
	{
		if (getParticles)
		{
			string @string = Res.GetString("{0}as well as", " ");
