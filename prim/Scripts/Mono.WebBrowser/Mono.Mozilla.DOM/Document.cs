...
internal class Document : Node, IDocument, INode
{
	...
	public string Charset
	{
		get
		{
			((nsIDOMDocumentView)node).getDefaultView(out var ret);
			((nsIInterfaceRequestor)ret).getInterface(typeof(nsIDocCharset).GUID, out var result);
			nsIDocCharset obj = (nsIDocCharset)Marshal.GetObjectForIUnknown(result);
			IntPtr ret2 = Marshal.StringToHGlobalUni(new StringBuilder(30).ToString());
			obj.getCharset(ref ret2);
		...
		set
		{
			((nsIDOMDocumentView)node).getDefaultView(out var ret);
			((nsIInterfaceRequestor)ret).getInterface(typeof(nsIDocCharset).GUID, out var result);
			((nsIDocCharset)Marshal.GetTypedObjectForIUnknown(result, typeof(nsIDocCharset))).setCharset(value);
	...
	public IStylesheetList Stylesheets
	{
		get
		{
			((nsIDOMDocumentStyle)node).getStyleSheets(out var ret);
