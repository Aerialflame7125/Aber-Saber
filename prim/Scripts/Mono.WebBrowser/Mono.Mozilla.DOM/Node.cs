...
internal class Node : DOMObject, INode
{
	...
	public string Style
	{
		...
		{
			node.getOwnerDocument(out var ret);
			((nsIDOMDocumentView)ret).getDefaultView(out var ret2);
			nsIDOMViewCSS obj = (nsIDOMViewCSS)ret2;
			Base.StringSet(storage, string.Empty);
			obj.getComputedStyle(pseudoElt: new AsciiString(string.Empty).Handle, elt: node as nsIDOMElement, ret: out var ret3);
			if (ret3 == null)
			{
				return "";
			}
		...
		set
		{
			...
			nsIDOMViewCSS obj = (nsIDOMViewCSS)ret2;
			Base.StringSet(storage, string.Empty);
			obj.getComputedStyle(node as nsIDOMElement, storage, out var ret3);
	...
	public virtual Mono.WebBrowser.DOM.NodeType Type
	{
		get
		{
			...
			return (Mono.WebBrowser.DOM.NodeType)Enum.ToObject(typeof(Mono.WebBrowser.DOM.NodeType), ret);
		}
	}
