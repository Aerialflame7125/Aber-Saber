...
internal class AnimationOutputWeightProcessor : ITimelineEvaluateCallback
{
	...
	private void FindMixers(Playable parent, int port, Playable node)
	{
		...
		if ((object)playableType == typeof(AnimationMixerPlayable) || (object)playableType == typeof(AnimationLayerMixerPlayable))
		{
			...
			weightInfo.modulate = (object)playableType == typeof(AnimationLayerMixerPlayable);
