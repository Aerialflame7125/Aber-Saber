...
public class ParticleControlPlayable : PlayableBehaviour
{
	...
	public void Initialize(ParticleSystem ps, uint randomSeed)
	{
		...
		particleSystem.randomSeed = m_RandomSeed;
	}
	...
	public override void PrepareFrame(Playable playable, FrameData data)
	{
		...
		if (particleSystem.randomSeed != m_RandomSeed)
		{
			particleSystem.Stop(withChildren: true, ParticleSystemStopBehavior.StopEmittingAndClear);
			particleSystem.useAutoRandomSeed = false;
			particleSystem.randomSeed = m_RandomSeed;
		...
		float num7 = ((!(num3 > num6)) ? (m_SystemTime - num5) : m_SystemTime);
		...
		{
			float num8 = ((!(num3 > num6)) ? num6 : particleSystem.main.duration);
