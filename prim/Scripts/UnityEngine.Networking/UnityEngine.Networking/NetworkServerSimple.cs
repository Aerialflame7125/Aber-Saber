...
public class NetworkServerSimple
{
	...
	public void ListenRelay(string relayIp, int relayPort, NetworkID netGuid, SourceID sourceId, NodeID nodeId)
	{
		...
		Update();
		NetworkTransport.ConnectAsNetworkHost(m_ServerHostId, relayIp, relayPort, netGuid, sourceId, nodeId, out var _);
		m_RelaySlotId = 0;
		if (LogFilter.logDebug)
		{
	...
	public void Update()
	{
		...
		do
		{
			networkEventType = NetworkTransport.ReceiveFromHost(m_ServerHostId, out var connectionId, out var channelId, m_MsgBuffer, m_MsgBuffer.Length, out var receivedSize, out error);
			if (networkEventType != NetworkEventType.Nothing && LogFilter.logDev)
			{
				Debug.Log(string.Concat("Server event: host=", m_ServerHostId, " event=", networkEventType, " error=", error));
			}
	...
	private void HandleConnect(int connectionId, byte error)
	{
		...
		NetworkTransport.GetConnectionInfo(m_ServerHostId, connectionId, out var address, out var _, out var _, out var _, out var error2);
