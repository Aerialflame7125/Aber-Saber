...
public class NetworkClient
{
	...
	private void ConnectWithRelay(MatchInfo info)
	{
		m_AsyncConnect = ConnectState.Connecting;
		Update();
		m_ClientConnectionId = NetworkTransport.ConnectToNetworkPeer(m_ClientId, info.address, info.port, 0, 0, info.networkId, Utility.GetSourceID(), info.nodeId, out var error);
		m_Connection = (NetworkConnection)Activator.CreateInstance(m_NetworkConnectionClass);
		m_Connection.SetHandlers(m_MessageHandlers);
	...
	internal virtual void Update()
	{
		...
		do
		{
			networkEventType = NetworkTransport.ReceiveFromHost(m_ClientId, out var _, out var channelId, m_MsgBuffer, (ushort)m_MsgBuffer.Length, out var receivedSize, out var error);
			if (m_Connection != null)
			{
			...
			{
				Debug.Log(string.Concat("Client event: host=", m_ClientId, " event=", networkEventType, " error=", error));
			}
