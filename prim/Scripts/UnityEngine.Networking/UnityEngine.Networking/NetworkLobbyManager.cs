...
public class NetworkLobbyManager : NetworkManager
{
	...
	private void SceneLoadedForPlayer(NetworkConnection conn, GameObject lobbyPlayerGameObject)
	{
		...
		{
			Debug.Log("NetworkLobby SceneLoadedForPlayer scene:" + text + " " + conn);
		}
	...
	public override void OnServerConnect(NetworkConnection conn)
	{
		if (base.numPlayers > maxPlayers)
		{
			...
			{
				Debug.LogWarning(string.Concat("NetworkLobbyManager can't accept new connection [", conn, "], too many players connected."));
			}
			conn.Disconnect();
			return;
		}
		...
		if (text != m_LobbyScene)
		{
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("NetworkLobbyManager can't accept new connection [", conn, "], not in lobby and game already in progress."));
			}
			conn.Disconnect();
			return;
		}
		...
		if (sceneName == m_LobbyScene)
		{
			for (int i = 0; i < lobbySlots.Length; i++)
			{
				...
				if (!(networkLobbyPlayer == null))
				{
					...
					if (component.connectionToClient.GetPlayerController(component.playerControllerId, out var playerController))
					{
	...
	private void OnServerReadyToBeginMessage(NetworkMessage netMsg)
	{
		...
		if (!netMsg.conn.GetPlayerController(s_ReadyToBeginMessage.slotId, out var playerController))
		{
	...
	private void OnServerSceneLoadedMessage(NetworkMessage netMsg)
	{
		...
		if (!netMsg.conn.GetPlayerController((short)s_SceneLoadedMessage.value, out var playerController))
		{
			if (LogFilter.logError)
			{
