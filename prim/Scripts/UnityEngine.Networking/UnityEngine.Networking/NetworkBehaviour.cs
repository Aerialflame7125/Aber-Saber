...
public class NetworkBehaviour : MonoBehaviour
{
	...
	protected void SendCommandInternal(NetworkWriter writer, int channelId, string cmdName)
	{
		...
		else if (ClientScene.readyConnection == null)
		{
			...
			{
				Debug.LogError(string.Concat("Send command attempted with no client running [client=", connectionToServer, "]."));
			}
		}
		else
		{
	...
	internal static void DumpInvokers()
	{
		...
		{
			Debug.Log(string.Concat("  Invoker:", s_CmdHandlerDelegate.Value.invokeClass, ":", s_CmdHandlerDelegate.Value.invokeFunction.GetMethodName(), " ", s_CmdHandlerDelegate.Value.invokeType, " ", s_CmdHandlerDelegate.Key));
		}
	}
	...
	internal bool InvokeCommandDelegate(int cmdHash, NetworkReader reader)
	{
		...
		if ((object)GetType() != invoker.invokeClass && !GetType().IsSubclassOf(invoker.invokeClass))
		{
			return false;
		}
	...
	internal bool InvokeSyncListDelegate(int cmdHash, NetworkReader reader)
	{
		...
		if ((object)GetType() != invoker.invokeClass)
		{
			return false;
		}
		invoker.invokeFunction(this, reader);
	...
	protected void SetSyncVarGameObject(GameObject newGameObject, ref GameObject gameObjectField, uint dirtyBit, ref NetworkInstanceId netIdField)
	{
		...
		if (newGameObject != null)
		{
			...
			if (component != null)
			{
				...
				{
					Debug.LogWarning(string.Concat("SetSyncVarGameObject GameObject ", newGameObject, " has a zero netId. Maybe it is not spawned yet?"));
				}
			}
		}
