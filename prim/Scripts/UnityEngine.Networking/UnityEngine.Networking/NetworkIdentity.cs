...
public sealed class NetworkIdentity : MonoBehaviour
{
	...
	internal void SetDynamicAssetId(NetworkHash128 newAssetId)
	{
		...
		{
			Debug.LogWarning(string.Concat("SetDynamicAssetId object already has an assetId <", m_AssetId, ">"));
		}
	}
	...
	internal void OnStartServer(bool allowNonZeroNetId)
	{
		...
		else if (!allowNonZeroNetId)
		{
			...
			{
				Debug.LogError(string.Concat("Object has non-zero netId ", netId, " for ", base.gameObject));
			}
			return;
		}
		if (LogFilter.logDev)
		{
			Debug.Log(string.Concat("OnStartServer ", base.gameObject, " GUID:", netId));
		}
	...
	internal void OnStartClient()
	{
		...
		if (LogFilter.logDev)
		{
			Debug.Log(string.Concat("OnStartClient ", base.gameObject, " GUID:", netId, " localPlayerAuthority:", localPlayerAuthority));
		}
		for (int i = 0; i < m_NetworkBehaviours.Length; i++)
		{
			NetworkBehaviour networkBehaviour = m_NetworkBehaviours[i];
	...
	internal void HandleClientAuthority(bool authority)
	{
		if (!localPlayerAuthority)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("HandleClientAuthority ", base.gameObject, " does not have localPlayerAuthority"));
			}
		}
		else
		{
	...
	private bool GetInvokeComponent(int cmdHash, Type invokeClass, out NetworkBehaviour invokeComponent)
	{
		...
		for (int i = 0; i < m_NetworkBehaviours.Length; i++)
		{
			...
			if ((object)networkBehaviour2.GetType() == invokeClass || networkBehaviour2.GetType().IsSubclassOf(invokeClass))
			{
		...
		if (networkBehaviour == null)
		{
			...
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("Found no behaviour for incoming [", cmdHashHandlerName, "] on ", base.gameObject, ",  the server and client should have the same NetworkBehaviour instances [netId=", netId, "]."));
			}
	...
	internal void HandleSyncEvent(int cmdHash, NetworkReader reader)
	{
		...
		if (base.gameObject == null)
		{
			...
			{
				Debug.LogWarning(string.Concat("SyncEvent [", cmdHashHandlerName, "] received for deleted object [netId=", netId, "]"));
			}
		}
		else if (!NetworkBehaviour.GetInvokerForHashSyncEvent(cmdHash, out invokeClass, out invokeFunction))
		{
			...
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("Found no receiver for incoming [", cmdHashHandlerName2, "] on ", base.gameObject, ",  the server and client should have the same NetworkBehaviour instances [netId=", netId, "]."));
			}
		}
		else if (!GetInvokeComponent(cmdHash, invokeClass, out invokeComponent))
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("SyncEvent [", cmdHashHandlerName3, "] handler not found [netId=", netId, "]"));
			}
		}
		else
		{
	...
	internal void HandleSyncList(int cmdHash, NetworkReader reader)
	{
		...
		if (base.gameObject == null)
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("SyncList [", cmdHashHandlerName, "] received for deleted object [netId=", netId, "]"));
			}
		}
		...
		else if (!GetInvokeComponent(cmdHash, invokeClass, out invokeComponent))
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("SyncList [", cmdHashHandlerName3, "] handler not found [netId=", netId, "]"));
			}
		}
		else
		{
	...
	internal void HandleCommand(int cmdHash, NetworkReader reader)
	{
		...
		if (base.gameObject == null)
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("Command [", cmdHashHandlerName, "] received for deleted object [netId=", netId, "]"));
			}
		}
		...
		else if (!GetInvokeComponent(cmdHash, invokeClass, out invokeComponent))
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("Command [", cmdHashHandlerName3, "] handler not found [netId=", netId, "]"));
			}
		}
		else
		{
	...
	internal void HandleRPC(int cmdHash, NetworkReader reader)
	{
		...
		if (base.gameObject == null)
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("ClientRpc [", cmdHashHandlerName, "] received for deleted object [netId=", netId, "]"));
			}
		}
		...
		else if (!GetInvokeComponent(cmdHash, invokeClass, out invokeComponent))
		{
			...
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("ClientRpc [", cmdHashHandlerName3, "] handler not found [netId=", netId, "]"));
			}
		}
		else
		{
	...
	internal void UNetUpdate()
	{
		...
		for (int j = 0; j < NetworkServer.numChannels; j++)
		{
			...
			for (int k = 0; k < m_NetworkBehaviours.Length; k++)
			{
				...
				{
					Debug.LogWarning(string.Concat("Large state update of ", s_UpdateWriter.Position - position, " bytes for netId:", netId, " from script:", networkBehaviour2));
				}
			}
			if (flag)
			{
	...
	internal void AddObserver(NetworkConnection conn)
	{
		if (m_Observers == null)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("AddObserver for ", base.gameObject, " observer list is null"));
			}
			return;
		}
	...
	public void RebuildObservers(bool initialize)
	{
		...
		foreach (NetworkConnection item in hashSet)
		{
			...
			if (!item.isReady)
			{
				if (LogFilter.logWarn)
				{
					Debug.LogWarning(string.Concat("Observer is not ready for ", base.gameObject, " ", item));
				}
			}
			else if (initialize || !hashSet2.Contains(item))
			{
				...
				if (LogFilter.logDebug)
				{
					Debug.Log(string.Concat("New Observer for ", base.gameObject, " ", item));
				}
				flag = true;
			}
		}
		foreach (NetworkConnection item2 in hashSet2)
		{
			if (!hashSet.Contains(item2))
			{
				...
				if (LogFilter.logDebug)
				{
					Debug.Log(string.Concat("Removed Observer for ", base.gameObject, " ", item2));
				}
				flag = true;
			}
		}
	...
	public bool RemoveClientAuthority(NetworkConnection conn)
	{
		...
		if (m_ClientAuthorityOwner == null)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("RemoveClientAuthority for ", base.gameObject, " has no clientAuthority owner."));
			}
			return false;
		}
		if (m_ClientAuthorityOwner != conn)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("RemoveClientAuthority for ", base.gameObject, " has different owner."));
			}
			return false;
		}
	...
	public bool AssignClientAuthority(NetworkConnection conn)
	{
		...
		if (m_ClientAuthorityOwner != null && conn != m_ClientAuthorityOwner)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("AssignClientAuthority for ", base.gameObject, " already has an owner. Use RemoveClientAuthority() first."));
			}
			return false;
		}
		if (conn == null)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("AssignClientAuthority for ", base.gameObject, " owner cannot be null. Use RemoveClientAuthority() instead."));
			}
			return false;
		}
		m_ClientAuthorityOwner = conn;
