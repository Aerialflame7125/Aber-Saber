...
public class NetworkTransform : NetworkBehaviour
{
	...
	private void UnserializeModeCharacterController(NetworkReader reader, bool initialState)
	{
		...
		Vector3 vector = m_TargetSyncPosition - base.transform.position;
		Vector3 vector2 = vector / GetNetworkSendInterval();
		m_FixedPosDiff = vector2 * Time.fixedDeltaTime;
		if (base.isServer && !base.isClient)
		{
			base.transform.position = m_TargetSyncPosition;
	...
	private void InterpolateTransformMode2D()
	{
		...
		if (interpolateRotation != 0f)
		{
			...
			m_RigidBody2D.MoveRotation(quaternion.eulerAngles.z);
	...
	public static void HandleTransform(NetworkMessage netMsg)
	{
		...
		{
			Debug.LogWarning(string.Concat("HandleTransform netId:", networkInstanceId, " is not for a valid player"));
		}
	}
