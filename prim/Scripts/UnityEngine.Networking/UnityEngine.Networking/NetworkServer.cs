...
public sealed class NetworkServer
{
	...
	internal void ActivateLocalClientScene()
	{
		...
		foreach (NetworkIdentity value in objects.Values)
		{
			if (!value.isClient)
			{
				if (LogFilter.logDev)
				{
					Debug.Log(string.Concat("ActivateClientScene ", value.netId, " ", value.gameObject));
				}
	...
	public static void SendBytesToReady(GameObject contextObj, byte[] buffer, int numBytes, int channelId)
	{
		...
		catch (NullReferenceException)
		{
			...
			{
				Debug.LogWarning(string.Concat("SendBytesToReady object ", contextObj, " has not been spawned"));
			}
		}
	}
	...
	public static void GetStatsOut(out int numMsgs, out int numBufferedMsgs, out int numBytes, out int lastBufferedPerSecond)
	{
		...
		for (int i = 0; i < connections.Count; i++)
		{
			...
			if (networkConnection != null)
			{
				networkConnection.GetStatsOut(out var numMsgs2, out var numBufferedMsgs2, out var numBytes2, out var lastBufferedPerSecond2);
				numMsgs += numMsgs2;
	...
	public static void GetStatsIn(out int numMsgs, out int numBytes)
	{
		...
		for (int i = 0; i < connections.Count; i++)
		{
			...
			if (networkConnection != null)
			{
				networkConnection.GetStatsIn(out var numMsgs2, out var numBytes2);
				numMsgs += numMsgs2;
				numBytes += numBytes2;
			}
		}
	...
	{
		if (GetNetworkIdentity(player, out var view))
		{
	...
	{
		if (!GetNetworkIdentity(playerGameObject, out var view))
		{
			if (LogFilter.logError)
			{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("Adding new playerGameObject object netId: ", playerGameObject.GetComponent<NetworkIdentity>().netId, " asset ID ", playerGameObject.GetComponent<NetworkIdentity>().assetId));
		}
	...
	private bool SetupLocalPlayerForConnection(NetworkConnection conn, NetworkIdentity uv, PlayerController newPlayerController)
	{
		...
		if (conn is ULocalConnectionToClient uLocalConnectionToClient)
		{
			if (LogFilter.logDev)
			{
	...
	private static void OnCommandMessage(NetworkMessage netMsg)
	{
		...
		if (gameObject == null)
		{
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("Instance not found when handling Command message [netId=", networkInstanceId, "]"));
			}
			return;
		}
		...
		if (component == null)
		{
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("NetworkIdentity deleted when handling Command message [netId=", networkInstanceId, "]"));
			}
			return;
		}
		bool flag = false;
		...
		if (!flag && component.clientAuthorityOwner != netMsg.conn)
		{
			if (LogFilter.logWarn)
			{
				Debug.LogWarning(string.Concat("Command for object without authority [netId=", networkInstanceId, "]"));
			}
			return;
		}
		if (LogFilter.logDev)
		{
			Debug.Log(string.Concat("OnCommandMessage for netId=", networkInstanceId, " conn=", netMsg.conn));
		}
	...
	internal void SpawnObject(GameObject obj)
	{
		if (!active)
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("SpawnObject for ", obj, ", NetworkServer is not active. Cannot spawn objects without an active server."));
			}
			return;
		}
		if (!GetNetworkIdentity(obj, out var view))
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("SpawnObject ", obj, " has no NetworkIdentity. Please add a NetworkIdentity to ", obj));
			}
			return;
		}
		view.Reset();
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("SpawnObject instance ID ", view.netId, " asset ID ", view.assetId));
		}
