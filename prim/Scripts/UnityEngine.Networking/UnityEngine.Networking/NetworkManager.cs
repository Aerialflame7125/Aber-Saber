...
public class NetworkManager : MonoBehaviour
{
	...
	private void InitializeSingleton()
	{
		...
		int num = (int)m_LogLevel;
		if (num != -1)
		{
			LogFilter.currentLogLevel = num;
		}
	...
	private bool StartServer(MatchInfo info, ConnectionConfig config, int maxConnections)
	{
		...
		if (!string.IsNullOrEmpty(m_OnlineScene) && m_OnlineScene != text && m_OnlineScene != m_OfflineScene)
		{
	...
	internal void OnServerRemovePlayerMessageInternal(NetworkMessage netMsg)
	{
		...
		netMsg.conn.GetPlayerController(s_RemovePlayerMessage.playerControllerId, out var playerController);
	...
	internal void OnClientConnectInternal(NetworkMessage netMsg)
	{
		...
		netMsg.conn.SetMaxDelay(m_MaxDelay);
		string text = SceneManager.GetSceneAt(0).name;
		if (string.IsNullOrEmpty(m_OnlineScene) || m_OnlineScene == m_OfflineScene || text == m_OnlineScene)
		{
