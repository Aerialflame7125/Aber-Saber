...
public class NetworkDiscovery : MonoBehaviour
{
	...
	public bool StartAsClient()
	{
		...
		NetworkTransport.SetBroadcastCredentials(m_HostId, m_BroadcastKey, m_BroadcastVersion, m_BroadcastSubVersion, out var _);
		m_Running = true;
		m_IsClient = true;
	...
	public bool StartAsServer()
	{
		...
		if (!NetworkTransport.StartBroadcastDiscovery(m_HostId, m_BroadcastPort, m_BroadcastKey, m_BroadcastVersion, m_BroadcastSubVersion, m_MsgOutBuffer, m_MsgOutBuffer.Length, m_BroadcastInterval, out var error))
		{
			if (LogFilter.logError)
			{
	...
	private void Update()
	{
		...
		do
		{
			networkEventType = NetworkTransport.ReceiveFromHost(m_HostId, out var _, out var _, m_MsgInBuffer, 1024, out var receivedSize, out var error);
			if (networkEventType == NetworkEventType.BroadcastEvent)
			{
				...
				NetworkTransport.GetBroadcastConnectionInfo(m_HostId, out var address, out var _, out error);
		...
		if (m_Running)
		{
			...
			}
			{
				foreach (string key in m_BroadcastsReceived.Keys)
				{
					...
					{
						string text2 = BytesToString(networkBroadcastResult.broadcastData);
