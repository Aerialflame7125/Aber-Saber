...
public class ClientScene
{
	...
	public static bool AddPlayer(NetworkConnection readyConn, short playerControllerId, MessageBase extraMessage)
	{
		...
		if (s_ReadyConnection.GetPlayerController(playerControllerId, out var playerController) && playerController.IsValid && playerController.gameObject != null)
		{
			if (LogFilter.logError)
			{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("ClientScene::AddPlayer() for ID ", playerControllerId, " called with connection [", s_ReadyConnection, "]"));
		}
	...
	public static bool RemovePlayer(short playerControllerId)
	{
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("ClientScene::RemovePlayer() for ID ", playerControllerId, " called with connection [", s_ReadyConnection, "]"));
		}
	...
	public static bool Ready(NetworkConnection conn)
	{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("ClientScene::Ready() called with connection [", conn, "]"));
		}
		if (conn != null)
		{
	...
	private static void OnObjectSpawn(NetworkMessage netMsg)
	{
		...
		if (!s_ObjectSpawnMessage.assetId.IsValid())
		{
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("OnObjSpawn netId: ", s_ObjectSpawnMessage.netId, " has invalid asset Id"));
			}
			return;
		}
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("Client spawn handler instantiating [netId:", s_ObjectSpawnMessage.netId, " asset ID:", s_ObjectSpawnMessage.assetId, " pos:", s_ObjectSpawnMessage.position, "]"));
		}
		GameObject prefab;
		...
		else if (NetworkScene.GetPrefab(s_ObjectSpawnMessage.assetId, out prefab))
		{
			...
			if (LogFilter.logDebug)
			{
				Debug.Log(string.Concat("Client spawn handler instantiating [netId:", s_ObjectSpawnMessage.netId, " asset ID:", s_ObjectSpawnMessage.assetId, " pos:", s_ObjectSpawnMessage.position, " rotation: ", s_ObjectSpawnMessage.rotation, "]"));
			}
			...
			if (uv == null)
			{
				if (LogFilter.logError)
				{
					Debug.LogError(string.Concat("Client object spawned for ", s_ObjectSpawnMessage.assetId, " does not have a NetworkIdentity"));
				}
			}
			else
			{
		...
		else if (NetworkScene.GetSpawnHandler(s_ObjectSpawnMessage.assetId, out handler))
		{
			...
			if (gameObject2 == null)
			{
				if (LogFilter.logWarn)
				{
					Debug.LogWarning(string.Concat("Client spawn handler for ", s_ObjectSpawnMessage.assetId, " returned null"));
				}
				return;
			}
			...
			if (uv == null)
			{
				if (LogFilter.logError)
				{
					Debug.LogError(string.Concat("Client object spawned for ", s_ObjectSpawnMessage.assetId, " does not have a network identity"));
				}
			}
			else
			{
		...
		{
			Debug.LogError(string.Concat("Failed to spawn server object, did you forget to add it to the NetworkManager? assetId=", s_ObjectSpawnMessage.assetId, " netId=", s_ObjectSpawnMessage.netId));
		}
	}
	...
	private static void OnObjectSpawnScene(NetworkMessage netMsg)
	{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("Client spawn scene handler instantiating [netId:", s_ObjectSpawnSceneMessage.netId, " sceneId:", s_ObjectSpawnSceneMessage.sceneId, " pos:", s_ObjectSpawnSceneMessage.position));
		}
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("Client spawn for [netId:", s_ObjectSpawnSceneMessage.netId, "] [sceneId:", s_ObjectSpawnSceneMessage.sceneId, "] obj:", networkIdentity.gameObject.name));
		}
	...
	private static void OnUpdateVarsMessage(NetworkMessage netMsg)
	{
		...
		if (LogFilter.logDev)
		{
			Debug.Log(string.Concat("ClientScene::OnUpdateVarsMessage ", networkInstanceId, " channel:", netMsg.channelId));
		}
