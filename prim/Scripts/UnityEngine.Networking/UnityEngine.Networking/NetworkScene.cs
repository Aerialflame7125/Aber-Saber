...
internal class NetworkScene
{
	...
	internal void SetLocalObject(NetworkInstanceId netId, GameObject obj, bool isClient, bool isServer)
	{
		if (LogFilter.logDev)
		{
			Debug.Log(string.Concat("SetLocalObject ", netId, " ", obj));
		}
		if (obj == null)
		{
	...
	internal static void RegisterSpawnHandler(NetworkHash128 assetId, SpawnDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
	{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("RegisterSpawnHandler asset '", assetId, "' ", spawnHandler.GetMethodName(), "/", unspawnHandler.GetMethodName()));
		}
	...
	internal static void RegisterPrefab(GameObject prefab, SpawnDelegate spawnHandler, UnSpawnDelegate unspawnHandler)
	{
		...
		if (LogFilter.logDebug)
		{
			Debug.Log(string.Concat("Registering custom prefab '", prefab.name, "' as asset:", component.assetId, " ", spawnHandler.GetMethodName(), "/", unspawnHandler.GetMethodName()));
		}
	...
	internal void DumpAllClientObjects()
	{
		foreach (NetworkInstanceId key in m_LocalObjects.Keys)
		{
			...
			if (networkIdentity != null)
			{
				Debug.Log(string.Concat("ID:", key, " OBJ:", networkIdentity.gameObject, " AS:", networkIdentity.assetId));
			}
			else
			{
				Debug.Log(string.Concat("ID:", key, " OBJ: null"));
			}
		}
	}
}
