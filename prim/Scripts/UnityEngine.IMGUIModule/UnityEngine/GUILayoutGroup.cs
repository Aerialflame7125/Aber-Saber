		...
		if (isVertical)
		{
			foreach (GUILayoutEntry entry in entries)
			{
				...
				if (entry.style != GUILayoutUtility.spaceStyle)
				{
					...
					m_ChildMinWidth = Mathf.Max(entry.minWidth + (float)rectOffset.horizontal, m_ChildMinWidth);
					m_ChildMaxWidth = Mathf.Max(entry.maxWidth + (float)rectOffset.horizontal, m_ChildMaxWidth);
				}
