...
public class AVProLiveCameraDevice : IDisposable
{
	...
	public int NumModes => _modes.Count;

	public int NumSettings => _settings.Count;

	public int NumVideoInputs => _videoInputs.Count;
	...
	public string CurrentFormat => _format;

	public string CurrentDeviceFormat => _deviceFormat;

	public float CurrentFrameRate => _frameRate;

	public long CurrentFrameDurationHNS => _frameDurationHNS;
	...
	private void EnumModes()
	{
		...
		for (int i = 0; i < numModes; i++)
		{
			if (AVProLiveCameraPlugin.GetModeInfo(_deviceIndex, i, out var width, out var height, out var fps, out var format))
			{
	...
	private void EnumVideoInputs()
	{
		...
		{
			if (AVProLiveCameraPlugin.GetVideoInputName(_deviceIndex, i, out var name))
			{
	...
	private void EnumVideoSettings()
	{
		...
		{
			if (!AVProLiveCameraPlugin.GetDeviceVideoSettingInfo(_deviceIndex, i, out var settingType, out var dataType, out var name, out var canAutomatic))
			{
				continue;
			}
			...
			case 0:
			{
				if (AVProLiveCameraPlugin.GetDeviceVideoSettingBoolean(_deviceIndex, i, out var defaultValue2, out var currentValue2, out var isAutomatic2))
				{
			...
			case 1:
			{
				if (AVProLiveCameraPlugin.GetDeviceVideoSettingFloat(_deviceIndex, i, out var defaultValue, out var currentValue, out var minValue, out var maxValue, out var isAutomatic))
				{
