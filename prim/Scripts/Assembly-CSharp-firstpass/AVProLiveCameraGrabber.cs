...
public class AVProLiveCameraGrabber : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (lastFrame != _lastFrame)
		{
			...
			if (AVProLiveCameraPlugin.GetFrameAsColor32(_device.DeviceIndex, _framePointer, _frameWidth, _frameHeight))
			{
				...
				_testTexture.Apply(updateMipmaps: false, makeNoLongerReadable: false);
			}
		}
	}
	...
	private void CreateBuffer(int width, int height)
	{
		...
		if (_frameData == null)
		{
			...
			_testTexture = new Texture2D(_frameWidth, _frameHeight, TextureFormat.ARGB32, mipmap: false, linear: false);
			_testTexture.Apply(updateMipmaps: false, makeNoLongerReadable: false);
		}
	}
	...
	private void OnGUI()
	{
		...
		{
			GUI.depth = 1;
			GUI.DrawTexture(new Rect(0f, 0f, Screen.width, Screen.height), _testTexture, ScaleMode.ScaleToFit, alphaBlend: false);
		}
	}
}
