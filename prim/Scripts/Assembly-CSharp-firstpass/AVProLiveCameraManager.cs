...
public class AVProLiveCameraManager : MonoBehaviour
{
	...
	private void AddNewDevices()
	{
		...
		{
			if (!AVProLiveCameraPlugin.GetDeviceGUID(i, out var text))
			{
				continue;
			}
			AVProLiveCameraDevice aVProLiveCameraDevice = FindDeviceWithGUID(text);
			if (aVProLiveCameraDevice == null && AVProLiveCameraPlugin.GetDeviceName(i, out var text2))
			{
	...
	private void EnumDevices()
	{
		...
		for (int i = 0; i < numDevices; i++)
		{
			if (AVProLiveCameraPlugin.GetDeviceName(i, out var text) && AVProLiveCameraPlugin.GetDeviceGUID(i, out var text2))
			{
				int numModes = AVProLiveCameraPlugin.GetNumModes(i);
				if (numModes > 0)
				{
