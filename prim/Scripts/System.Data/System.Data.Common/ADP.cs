...
internal static class ADP
{
	...
	{
		return Argument(global::SR.GetString("The parameter data type of {0} is invalid.", typeName));
	}
	...
	{
		string @string = global::SR.GetString("Failed to convert parameter value from a {0} to a {1}.", value.GetType().Name, destType.Name);
		if (inner is ArgumentException)
		{
			return new ArgumentException(@string, inner);
		}
		if (inner is FormatException)
		{
			return new FormatException(@string, inner);
		}
		if (inner is InvalidCastException)
		{
			return new InvalidCastException(@string, inner);
		}
		if (inner is OverflowException)
		{
			return new OverflowException(@string, inner);
		}
		return inner;
	}
