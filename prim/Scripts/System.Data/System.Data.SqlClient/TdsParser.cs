...
internal sealed class TdsParser
{
	...
	internal void Connect(ServerInfo serverInfo, SqlInternalConnectionTds connHandler, bool ignoreSniOpenTimeout, long timerExpire, bool encrypt, bool trustServerCert, bool integratedSecurity, bool withFailover)
	{
		...
		if (ConsumePreLoginHandshake(encrypt, trustServerCert, integratedSecurity, out marsCapable) == PreLoginHandshakeStatus.InstanceFailure)
		{
			_physicalStateObj.Dispose();
		...
		while (true)
		{
			...
			case 1:
			{
				...
				if (_encryptionOption == EncryptionOptions.ON || _encryptionOption == EncryptionOptions.LOGIN)
				{
					...
					num4 = _physicalStateObj.EnableSsl(ref info);
					if (num4 != 0)
					{
						_physicalStateObj.AddError(ProcessSNIError(_physicalStateObj));
						ThrowExceptionAndWarning(_physicalStateObj);
					}
					WaitForSSLHandShakeToComplete(ref num4);
					_physicalStateObj.ClearAllWritePackets();
				}
				goto IL_0345;
			}
	...
	internal void TdsLogin(SqlLogin rec, TdsEnums.FeatureExtension requestedFeatures, SessionData recoverySessionData)
	{
		...
		if (flag)
		{
			...
			{
				num2 += WriteSessionRecoveryFeatureRequest(recoverySessionData, write: false);
			}
			if ((requestedFeatures & TdsEnums.FeatureExtension.GlobalTransactions) != 0)
			{
				num2 += WriteGlobalTransactionsFeatureRequest(write: false);
			}
			num2++;
		}
		try
		{
			...
			WriteInt(0, _physicalStateObj);
			int num4 = 94;
			WriteShort(num4, _physicalStateObj);
			...
			if (flag)
			{
				if ((requestedFeatures & TdsEnums.FeatureExtension.SessionRecovery) != 0)
				{
					num2 += WriteSessionRecoveryFeatureRequest(recoverySessionData, write: true);
				}
				if ((requestedFeatures & TdsEnums.FeatureExtension.GlobalTransactions) != 0)
				{
			...
			try
			{
				...
				for (int ii = startRpc; ii < rpcArray.Length; ii++)
				{
					...
					for (i = ((ii == startRpc) ? startParam : 0); i < parameters.Length; i++)
					{
						...
						if (!_isKatmai && !internalMetaType.Is90Supported)
						{
						...
						if (internalMetaType.IsAnsiType)
						{
							...
							else
							{
								...
								if (num4 == 0)
								{
									num4 = ((!internalMetaType.IsNCharType) ? 1 : 2);
								}
								WriteParameterVarLen(internalMetaType, num4, isNull: false, stateObj);
							}
						}
						...
						if (internalMetaType.SqlDbType == SqlDbType.Decimal)
						{
							...
							if (!flag3)
							{
								if (flag4)
								{
									obj = AdjustSqlDecimalScale((SqlDecimal)obj, actualScale);
								...
								{
									obj = AdjustDecimalScale((decimal)obj, actualScale);
	...
	private void WriteSmiParameter(SqlParameter param, int paramIndex, bool sendDefault, TdsParserStateObject stateObj)
	{
		...
		else
		{
			...
			typeCode = MetaDataUtilsSmi.DetermineExtendedTypeCodeForUseWithSqlDbType(smiParameterMetaData.SqlDbType, smiParameterMetaData.IsMultiValued, value);
		}
	...
	private async Task WriteStreamFeed(StreamDataFeed feed, TdsParserStateObject stateObj, int len)
	{
		...
		do
		{
			...
			int nRead = ((!_asyncWrite) ? feed._source.Read(buff, 0, readSize) : (await feed._source.ReadAsync(buff, 0, readSize).ConfigureAwait(continueOnCapturedContext: false)));
			if (nRead == 0)
			{
				break;
			}
