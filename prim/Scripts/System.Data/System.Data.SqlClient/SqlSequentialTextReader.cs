...
internal sealed class SqlSequentialTextReader : TextReader
{
	...
	public override Task<int> ReadAsync(char[] buffer, int index, int count)
	{
		...
		else
		{
			...
			{
				completion.TrySetException(exception2);
	...
	private int InternalRead(char[] buffer, int index, int count)
	{
		try
		{
			...
			byteBufferUsed += _reader.GetBytesInternalSequential(_columnIndex, array, byteBufferUsed, array.Length - byteBufferUsed, null);
			if (byteBufferUsed > 0)
			{
