...
internal sealed class SqlInternalConnectionTds : SqlInternalConnection, IDisposable
{
		...
		: base(connectionOptions)
	{
		...
		try
		{
			...
			for (int i = 0; i < num; i++)
			{
				try
				{
					OpenLoginEnlist(timeoutTimer, connectionOptions, redirectedUserInstance);
					break;
				}
				catch (SqlException ex)
				{
					if (i + 1 == num || !applyTransientFaultHandling || timeoutTimer.IsExpired || timeoutTimer.MillisecondsRemaining < num2 || !IsTransientError(ex))
					{
						throw ex;
					}
					Thread.Sleep(num2);
	...
	internal void CheckEnlistedTransactionBinding()
	{
		...
		if (base.ConnectionOptions.TransactionBinding == SqlConnectionString.TransactionBindingEnum.ExplicitUnbind)
		{
			...
			if (enlistedTransaction.TransactionInformation.Status != 0 || !enlistedTransaction.Equals(current))
			{
				throw ADP.TransactionConnectionMismatch();
			}
		}
		else if (enlistedTransaction.TransactionInformation.Status != 0)
		{
	...
	private void Login(ServerInfo server, TimeoutTimer timeout)
	{
		...
		_parser.TdsLogin(sqlLogin, featureExtension, _recoverySessionData);
	}
	...
	private void OpenLoginEnlist(TimeoutTimer timeout, SqlConnectionString connectionOptions, bool redirectedUserInstance)
	{
		...
		try
		{
			...
			if (flag2)
			{
				...
				LoginWithFailover(flag, serverInfo, failoverPartner, redirectedUserInstance, connectionOptions, timeout);
			}
			else
			{
				_timeoutErrorInternal.SetFailoverScenario(useFailoverServer: false);
				LoginNoFailover(serverInfo, redirectedUserInstance, connectionOptions, timeout);
			}
	...
	private void LoginNoFailover(ServerInfo serverInfo, bool redirectedUserInstance, SqlConnectionString connectionOptions, TimeoutTimer timeout)
	{
		...
		while (true)
		{
			...
			try
			{
				AttemptOneLogin(serverInfo, !connectionOptions.MultiSubnetFailover, connectionOptions.MultiSubnetFailover ? timeoutTimer : timeout);
			...
			{
				if (_parser == null || _parser.State != 0 || IsDoNotRetryConnectError(exc) || timeout.IsExpired)
				{
					throw;
				}
			...
			if (ServerProvidedFailOverPartner != null)
			{
				...
				_timeoutErrorInternal.SetFailoverScenario(useFailoverServer: true);
				LoginWithFailover(useFailoverHost: true, serverInfo, ServerProvidedFailOverPartner, redirectedUserInstance, connectionOptions, timeout);
				return;
			}
			Thread.Sleep(num2);
	...
	private void LoginWithFailover(bool useFailoverHost, ServerInfo primaryServerInfo, string failoverHost, bool redirectedUserInstance, SqlConnectionString connectionOptions, TimeoutTimer timeout)
	{
		...
		while (true)
		{
			...
			try
			{
				AttemptOneLogin(serverInfo2, ignoreSniOpenTimeout: false, timeout2, withFailover: true);
				if (_routingInfo != null)
				{
