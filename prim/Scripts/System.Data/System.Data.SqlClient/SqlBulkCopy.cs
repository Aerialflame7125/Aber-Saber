...
public sealed class SqlBulkCopy : IDisposable
{
	...
	private string AnalyzeTargetAndCreateUpdateBulkCommand(BulkCopySimpleResultSet internalResults)
	{
		...
		stringBuilder.Append(")");
		if ((_copyOptions & (SqlBulkCopyOptions.CheckConstraints | SqlBulkCopyOptions.TableLock | SqlBulkCopyOptions.KeepNulls | SqlBulkCopyOptions.FireTriggers)) != 0)
		{
			bool flag3 = false;
	...
	private Task WriteRowSourceToServerAsync(int columnCount, CancellationToken ctoken)
	{
		...
		if (currentReconnectionTask != null && !currentReconnectionTask.IsCompleted)
		{
			if (_isAsyncBulkCopy)
			{
				...
				{
					Task task = WriteRowSourceToServerAsync(columnCount, ctoken);
					if (task == null)
					{
						tcs.SetResult(null);
					}
					...
					{
						AsyncHelper.ContinueTask(task, tcs, delegate
						{
							tcs.SetResult(null);
						});
					}
		...
		try
		{
			...
			Task task2 = WriteToServerInternalAsync(ctoken);
			if (task2 != null)
			{
				flag = false;
				return task2.ContinueWith(delegate(Task t)
				{
					AbortTransaction();
	...
	private void WriteRowSourceToServerCommon(int columnCount)
	{
		...
		foreach (SqlBulkCopyColumnMapping localColumnMapping2 in _localColumnMappings)
		{
			...
			case ValueSourceType.DbDataReader:
				try
				{
					num = _DbDataReaderRowSource.GetOrdinal(text);
				}
	...
	private bool FireRowsCopiedEvent(long rowsCopied)
	{
		...
		try
		{
			...
			OnRowsCopied(sqlRowsCopiedEventArgs);
		}
		finally
		{
