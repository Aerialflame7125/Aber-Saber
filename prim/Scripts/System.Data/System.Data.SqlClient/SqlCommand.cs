		...
		case CommandType.StoredProcedure:
		{
			...
			try
			{
				...
				while (sqlDataReader.Read())
				{
					...
					{
						sqlParameter.TypeName = string.Concat(sqlDataReader[array2[7]], ".", sqlDataReader[array2[8]], ".", sqlDataReader[array2[9]]);
					}
	...
	private SqlDataReader RunExecuteReaderTds(CommandBehavior cmdBehavior, RunBehavior runBehavior, bool returnStream, bool async, int timeout, out Task task, bool asyncWrite, SqlDataReader ds = null)
	{
			...
			if (async)
			{
				...
				AsyncHelper.ContinueTask(task2, completion, delegate
				{
					if (!completion.Task.IsCompleted)
					{
						Interlocked.CompareExchange(ref _reconnectionCompletionSource, null, completion);
						timeoutCTS.Cancel();
						RunExecuteReaderTds(cmdBehavior, runBehavior, returnStream, async, TdsParserStaticMethods.GetRemainingTimeout(timeout, reconnectionStart), out var task3, asyncWrite, ds);
						if (task3 == null)
						{
							completion.SetResult(null);
						}
						...
						{
							AsyncHelper.ContinueTask(task3, completion, delegate
							{
								completion.SetResult(null);
							});
						}
		...
		try
		{
			...
			if (CommandType.Text == CommandType && GetParameterCount(_parameters) == 0)
			{
				...
				task4 = _stateObj.Parser.TdsExecuteSQLBatch(text, timeout, Notification, _stateObj, !asyncWrite);
			}
			else if (CommandType.Text == CommandType)
			{
				...
				rpc.options = 2;
				task4 = _stateObj.Parser.TdsExecuteRPC(_rpcArrayOf1, timeout, inSchema, Notification, _stateObj, CommandType.StoredProcedure == CommandType, !asyncWrite);
			}
			else
			{
			...
			if (async)
			{
				...
				if (task4 != null)
				{
					task = AsyncHelper.CreateContinuationTask(task4, delegate
					{
	...
	internal string BuildParamList(TdsParser parser, SqlParameterCollection parameters)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (metaType.SqlDbType == SqlDbType.Structured)
			{
				...
				stringBuilder.Append(ParseAndQuoteIdentifier(typeName));
