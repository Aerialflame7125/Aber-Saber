...
internal abstract class OdbcHandle : SafeHandle
{
		...
		: base(IntPtr.Zero, ownsHandle: true)
	{
		...
		try
		{
			...
			case ODBC32.SQL_HANDLE.ENV:
				retCode = UnsafeNativeMethods.SQLAllocHandle(handleType, IntPtr.Zero, out handle);
				break;
			case ODBC32.SQL_HANDLE.DBC:
			case ODBC32.SQL_HANDLE.STMT:
				parentHandle.DangerousAddRef(ref success);
				retCode = UnsafeNativeMethods.SQLAllocHandle(handleType, parentHandle, out handle);
				break;
			}
		}
		finally
	...
	protected override bool ReleaseHandle()
	{
		...
		if (IntPtr.Zero != intPtr)
		{
			...
			case ODBC32.SQL_HANDLE.DBC:
			case ODBC32.SQL_HANDLE.STMT:
				UnsafeNativeMethods.SQLFreeHandle(handleType, intPtr);
				break;
			}
		}
	...
	internal ODBC32.RetCode GetDiagnosticField(out string sqlState)
	{
		...
		short StringLength;
		ODBC32.RetCode retCode = UnsafeNativeMethods.SQLGetDiagFieldW(HandleType, this, 1, 4, stringBuilder, checked((short)(2 * stringBuilder.Capacity)), out StringLength);
	...
	internal ODBC32.RetCode GetDiagnosticRecord(short record, out string sqlState, StringBuilder message, out int nativeError, out short cchActual)
	{
		...
		ODBC32.RetCode retCode = UnsafeNativeMethods.SQLGetDiagRecW(HandleType, this, record, stringBuilder, out nativeError, message, checked((short)message.Capacity), out cchActual);
