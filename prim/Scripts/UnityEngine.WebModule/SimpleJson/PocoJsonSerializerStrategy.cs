...
internal class PocoJsonSerializerStrategy : IJsonSerializerStrategy
{
	...
	public virtual object DeserializeObject(object value, Type type)
	{
		...
		if ((object)type == typeof(Guid) && string.IsNullOrEmpty(text))
		{
			return default(Guid);
		}
		if (value == null)
		...
		if (text != null)
		{
			if (text.Length != 0)
			{
				if ((object)type == typeof(DateTime) || (ReflectionUtils.IsNullableType(type) && (object)Nullable.GetUnderlyingType(type) == typeof(DateTime)))
				{
				...
				if ((object)type == typeof(DateTimeOffset) || (ReflectionUtils.IsNullableType(type) && (object)Nullable.GetUnderlyingType(type) == typeof(DateTimeOffset)))
				{
				...
				if ((object)type == typeof(Guid) || (ReflectionUtils.IsNullableType(type) && (object)Nullable.GetUnderlyingType(type) == typeof(Guid)))
				{
					return new Guid(text);
				}
				return text;
			}
			obj = (((object)type == typeof(Guid)) ? ((object)default(Guid)) : ((!ReflectionUtils.IsNullableType(type) || (object)Nullable.GetUnderlyingType(type) != typeof(Guid)) ? text : null));
			if (!ReflectionUtils.IsNullableType(type) && (object)Nullable.GetUnderlyingType(type) == typeof(Guid))
			{
				return text;
			}
		}
		...
		if ((flag && (object)type == typeof(long)) || (flag2 && (object)type == typeof(double)))
		{
			return value;
		}
		if ((flag2 && (object)type != typeof(double)) || (flag && (object)type != typeof(long)))
		{
