...
public sealed class ExpandoObject : IDynamicMetaObjectProvider, IDictionary<string, object>, ICollection<KeyValuePair<string, object>>, IEnumerable<KeyValuePair<string, object>>, IEnumerable, INotifyPropertyChanged
{
	...
	private class MetaExpando : DynamicMetaObject
	{
		...
		private DynamicMetaObject BindGetOrInvokeMember(DynamicMetaObjectBinder binder, string name, bool ignoreCase, DynamicMetaObject fallback, Func<DynamicMetaObject, DynamicMetaObject> fallbackInvoke)
		{
			...
			Expression test = Expression.Call(ExpandoTryGetValue, GetLimitedSelf(), Expression.Constant(@class, typeof(object)), System.Linq.Expressions.Utils.Constant(valueIndex), Expression.Constant(name), System.Linq.Expressions.Utils.Constant(ignoreCase), parameterExpression);
		...
		public override DynamicMetaObject BindDeleteMember(DeleteMemberBinder binder)
		{
			...
			MethodCallExpression expression = Expression.Call(ExpandoTryDeleteValue, GetLimitedSelf(), Expression.Constant(Value.Class, typeof(object)), System.Linq.Expressions.Utils.Constant(valueIndex), Expression.Constant(binder.Name), System.Linq.Expressions.Utils.Constant(binder.IgnoreCase));
		...
		private DynamicMetaObject AddDynamicTestAndDefer(DynamicMetaObjectBinder binder, ExpandoClass klass, ExpandoClass originalClass, DynamicMetaObject succeeds)
		{
			...
			{
				expression = Expression.Block(Expression.Call(null, ExpandoPromoteClass, GetLimitedSelf(), Expression.Constant(originalClass, typeof(object)), Expression.Constant(klass, typeof(object))), succeeds.Expression);
			}
			return new DynamicMetaObject(Expression.Condition(Expression.Call(null, ExpandoCheckVersion, GetLimitedSelf(), Expression.Constant(originalClass ?? klass, typeof(object))), expression, binder.GetUpdateExpression(expression.Type)), GetRestrictions().Merge(succeeds.Restrictions));
		}
