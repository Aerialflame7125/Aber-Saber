...
internal class GridEntry : GridItem, ITypeDescriptorContext, IServiceProvider
{
	...
	IContainer ITypeDescriptorContext.Container
	{
		get
		{
			...
			if (property_grid.SelectedObject is IComponent { Site: not null } component)
			{
	...
	public override string Label
	{
		get
		{
			...
			if (propertyDescriptor != null)
			{
				...
				if (propertyDescriptor.Attributes[typeof(ParenthesizePropertyNameAttribute)] is ParenthesizePropertyNameAttribute { NeedParenthesis: not false })
				{
					text = "(" + text + ")";
				}
				return text;
	...
	protected virtual bool SetValueCore(object value, out string error)
	{
		...
		if ((object)type != null && (object)PropertyDescriptor.PropertyType != null && !PropertyDescriptor.PropertyType.IsAssignableFrom(type))
		{
			bool flag = false;
			try
			{
	...
	private string[] GetMergedPropertyNames(object[] objects)
	{
		...
		for (int i = 0; i < objects.Length; i++)
		{
			...
			foreach (PropertyDescriptor item in (IEnumerable)obj)
			{
				...
				if ((objects.Length <= 1 || IsPropertyMergeable(propertyDescriptor2)) && (object)propertyDescriptor2.PropertyType == item.PropertyType)
				{
