...
internal class XplatUIWin32 : XplatUIDriver
{
	...
	internal override bool IsMinimizeRestoreAnimationEnabled
	{
		get
		{
			...
			value.cbSize = (uint)Marshal.SizeOf((object)value);
	...
	internal override IntPtr CreateWindow(CreateParams cp)
	{
		...
		if (intPtr == IntPtr.Zero && (cp.Style & 0x40000000) != 0)
		{
			intPtr = FosterParent;
		}
	...
	internal override void ResetMouseHover(IntPtr handle)
	{
		...
		tme.size = Marshal.SizeOf((object)tme);
		tme.hWnd = handle;
	...
	private bool GetMessage(ref MSG msg, IntPtr hWnd, int wFilterMin, int wFilterMax, bool blocking)
	{
		...
		case Msg.WM_MOUSEMOVE:
			if (msg.hwnd != prev_mouse_hwnd)
			{
				...
				tme2.size = Marshal.SizeOf((object)tme2);
				tme2.hWnd = msg.hwnd;
	...
	internal override void ScrollWindow(IntPtr hwnd, Rectangle rectangle, int XAmount, int YAmount, bool with_children)
	{
		...
		Win32ScrollWindowEx(hwnd, XAmount, YAmount, IntPtr.Zero, ref prcClip, IntPtr.Zero, IntPtr.Zero, (ScrollWindowExFlags)(6 | (with_children ? 1 : 0)));
		Win32UpdateWindow(hwnd);
	}

	internal override void ScrollWindow(IntPtr hwnd, int XAmount, int YAmount, bool with_children)
	{
		Win32ScrollWindowEx(hwnd, XAmount, YAmount, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, (ScrollWindowExFlags)(6 | (with_children ? 1 : 0)));
	}
	...
	internal override bool SystrayAdd(IntPtr hwnd, string tip, Icon icon, out ToolTip tt)
	{
		...
		lpData.cbSize = (uint)Marshal.SizeOf((object)lpData);
		lpData.hWnd = hwnd;
		lpData.uID = 1u;
