...
internal class XplatUIX11 : XplatUIDriver
{
		...
		get
		{
			if (XGetIconSizes(DisplayHandle, RootWindow, out var size_list, out var count) != 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					num += Marshal.SizeOf((object)xIconSize);
	...
	{
		if (cp.control is Form { window_manager: null } && !cp.IsSet(WindowExStyles.WS_EX_TOOLWINDOW))
		{
			...
			else
			{
				...
				case XEventName.KeyPress:
					...
					if (msg.wParam == (IntPtr)112 || msg.wParam == (IntPtr)47)
					{
						...
						IntPtr intPtr = Marshal.AllocHGlobal(Marshal.SizeOf((object)hELPINFO));
						Marshal.StructureToPtr((object)hELPINFO, intPtr, fDeleteOld: true);
	...
	internal override void ScreenToMenu(IntPtr handle, ref int x, ref int y)
	{
		...
		if (Control.FromHandle(handle) is Form { window_manager: not null } form)
		{
	...
	{
		if (Control.FromHandle(handle) is Form { window_manager: null } form)
		{
			CreateParams createParams = form.GetCreateParams();
