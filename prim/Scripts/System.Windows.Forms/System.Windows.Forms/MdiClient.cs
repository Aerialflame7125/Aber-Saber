...
public sealed class MdiClient : Control
{
	...
	public new class ControlCollection : Control.ControlCollection
	{
		...
		{
			if (value is Form { WindowManager: MdiWindowManager windowManager } form)
			{
				form.Closed -= windowManager.form_closed_handler;
			}
	...
	public void LayoutMdi(MdiLayout value)
	{
		...
		case MdiLayout.TileVertical:
		{
			...
			for (int i = 0; i < base.Controls.Count; i++)
			{
				if (!(base.Controls[i] is Form { Visible: not false } form))
				{
					continue;
				}
			...
			{
				if (base.Controls[j] is Form { Visible: not false, WindowState: not FormWindowState.Minimized } form2)
				{
					form2.Size = size;
