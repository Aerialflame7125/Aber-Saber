...
public class DateTimePicker : Control
{
	...
	public DateTime MaxDate
	{
		...
		set
		{
			if (value < min_date)
			{
				string message = string.Format(CultureInfo.CurrentCulture, "'{0}' is not a valid value for 'MaxDate'. 'MaxDate' must be greater than or equal to MinDate.", new object[1] { value.ToString("G") });
				throw new ArgumentOutOfRangeException("MaxDate", message);
			}
			if (value > MaxDateTime)
			{
				string message2 = string.Format(CultureInfo.CurrentCulture, "DateTimePicker does not support dates after {0}.", new object[1] { MaxDateTime.ToString("G", CultureInfo.CurrentCulture) });
	...
	public DateTime MinDate
	{
		...
		set
		{
			...
			if (value > MaxDate)
			{
				string message = string.Format(CultureInfo.CurrentCulture, "'{0}' is not a valid value for 'MinDate'. 'MinDate' must be less than MaxDate.", new object[1] { value.ToString("G") });
				throw new ArgumentOutOfRangeException("MinDate", message);
			}
			if (value < MinDateTime)
			{
				string message2 = string.Format(CultureInfo.CurrentCulture, "DateTimePicker does not support dates before {0}.", new object[1] { MinDateTime.ToString("G", CultureInfo.CurrentCulture) });
