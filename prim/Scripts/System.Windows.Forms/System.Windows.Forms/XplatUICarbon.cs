...
internal class XplatUICarbon : XplatUIDriver
{
	...
	internal void PerformNCCalc(Hwnd hwnd)
	{
		...
		IntPtr intPtr = Marshal.AllocHGlobal(Marshal.SizeOf((object)nCCALCSIZE_PARAMS));
		...
		Marshal.StructureToPtr((object)nCCALCSIZE_PARAMS, intPtr, fDeleteOld: true);
		...
		case Msg.WM_NCCALCSIZE:
			if (msg.WParam == (IntPtr)1)
			{
				...
				if (control != null)
				{
					...
					Marshal.StructureToPtr((object)nCCALCSIZE_PARAMS, msg.LParam, fDeleteOld: true);
				}
			}
			break;
	...
	internal override void SetIcon(IntPtr handle, Icon icon)
	{
		...
		for (int i = 0; i < bitmap.Height; i++)
		{
			for (int j = 0; j < bitmap.Width; j++)
			{
				...
				{
					byte b = (byte)((num3 >> 24) & 0xFF);
					byte b2 = (byte)((num3 >> 16) & 0xFF);
					byte b3 = (byte)((num3 >> 8) & 0xFF);
					byte b4 = (byte)(num3 & 0xFF);
	...
	{
		if (Control.FromHandle(handle) is Form { window_manager: null } form && (border_style == FormBorderStyle.FixedToolWindow || border_style == FormBorderStyle.SizableToolWindow))
		{
	...
	internal override void SetWindowState(IntPtr handle, FormWindowState state)
	{
		...
		case FormWindowState.Maximized:
			if (Control.FromHandle(hwnd.Handle) is Form { FormBorderStyle: FormBorderStyle.None })
			{
				Rect r = default(Rect);
	...
	internal override void SetWindowStyle(IntPtr handle, CreateParams cp)
	{
		...
		{
			WindowAttributes windowAttributes = (WindowAttributes)34078720u;
			if ((cp.Style & 0x20000) != 0)
			{
				windowAttributes |= WindowAttributes.kWindowCollapseBoxAttribute;
			}
			if ((cp.Style & 0x10000) != 0)
			{
				windowAttributes |= (WindowAttributes)22u;
			}
			if ((cp.Style & 0x80000) != 0)
			{
				windowAttributes |= WindowAttributes.kWindowCloseBoxAttribute;
			}
			if ((cp.ExStyle & 0x80) != 0)
			{
				windowAttributes = (WindowAttributes)34078720u;
			}
			windowAttributes |= WindowAttributes.kWindowLiveResizeAttribute;
