	...
	private class Number
	{
		...
		private unsafe static bool ParseNumber(ref char* str, NumberStyles options, ref NumberBuffer number, StringBuilder sb, NumberFormatInfo numfmt, bool parseDecimal)
		{
			...
			while (true)
			{
				...
				{
					char* ptr2;
					if ((options & NumberStyles.AllowLeadingSign) != 0 && (num & 1) == 0 && ((ptr2 = MatchChars(ptr, numfmt.PositiveSign)) != null || ((ptr2 = MatchChars(ptr, numfmt.NegativeSign)) != null && (number.sign = true))))
					{
						num |= 1;
						ptr = ptr2 - 1;
					}
			...
			while (true)
			{
				...
				{
					num |= 4;
					if (c != '0' || (num & 8) != 0 || (flag2 && (options & NumberStyles.AllowHexSpecifier) != 0))
					{
						if (num3 < num2)
						{
							if (flag2)
				...
				else if ((options & NumberStyles.AllowDecimalPoint) != 0 && (num & 0x10) == 0 && ((ptr2 = MatchChars(ptr, str2)) != null || (flag && (num & 0x20) == 0 && (ptr2 = MatchChars(ptr, numfmt.NumberDecimalSeparator)) != null)))
				{
					num |= 0x10;
					ptr = ptr2 - 1;
				}
				...
				{
					if ((options & NumberStyles.AllowThousands) == 0 || (num & 4) == 0 || (num & 0x10) != 0 || ((ptr2 = MatchChars(ptr, str3)) == null && (!flag || (num & 0x20) != 0 || (ptr2 = MatchChars(ptr, numfmt.NumberGroupSeparator)) == null)))
					{
						break;
					}
					ptr = ptr2 - 1;
			...
			if ((num & 4) != 0)
			{
				...
				while (true)
				{
					...
					{
						char* ptr2;
						if ((options & NumberStyles.AllowTrailingSign) != 0 && (num & 1) == 0 && ((ptr2 = MatchChars(ptr, numfmt.PositiveSign)) != null || ((ptr2 = MatchChars(ptr, numfmt.NegativeSign)) != null && (number.sign = true))))
						{
							num |= 1;
							ptr = ptr2 - 1;
						}
		...
		internal unsafe static string NumberToString(NumberBuffer number, char format, int nMaxDigits, NumberFormatInfo info, bool isDecimal)
		{
			...
			case 'c':
				...
				FormatCurrency(stringBuilder, number, num, nMaxDigits, info);
				break;
			case 'F':
			case 'f':
				if (nMaxDigits < 0)
				...
				FormatFixed(stringBuilder, number, num, nMaxDigits, info, null, info.NumberDecimalSeparator, null);
				break;
			case 'N':
			case 'n':
				if (nMaxDigits < 0)
				...
				RoundNumber(ref number, number.scale + nMaxDigits);
				FormatNumber(stringBuilder, number, num, nMaxDigits, info);
				break;
			case 'E':
			case 'e':
				if (nMaxDigits < 0)
				...
				FormatScientific(stringBuilder, number, num, nMaxDigits, info, format);
				break;
			case 'G':
			case 'g':
			{
				...
				FormatGeneral(stringBuilder, number, num, nMaxDigits, info, (char)(format - 2), !flag);
				break;
			}
			case 'P':
			case 'p':
				...
				number.scale += 2;
				RoundNumber(ref number, number.scale + nMaxDigits);
				FormatPercent(stringBuilder, number, num, nMaxDigits, info);
				break;
			default:
		...
		internal unsafe static string NumberToStringFormat(NumberBuffer number, string format, NumberFormatInfo info)
		{
			...
			bool flag3 = false;
			fixed (char* ptr3 = format)
			{
				char* ptr2 = digits;
				char c;
				while ((c = ptr3[num9++]) != 0 && c != ';')
				{
					if (num11 > 0 && (c == '#' || c == '.' || c == '0'))
					{
						while (num11 > 0)
						{
							stringBuilder.Append((*ptr2 != 0) ? (*(ptr2++)) : '0');
					...
					case '"':
					case '\'':
						while (ptr3[num9] != 0 && ptr3[num9] != c)
						{
							stringBuilder.Append(ptr3[num9++]);
						}
						if (ptr3[num9] != 0)
						{
							num9++;
					...
					case 'e':
					{
						...
						if (flag)
						{
							...
							else if (ptr3[num9] == '+' && ptr3[num9 + 1] == '0')
							{
								positiveSign = true;
							}
							else if (ptr3[num9] != '-' || ptr3[num9 + 1] != '0')
							...
							FormatExponent(stringBuilder, info, value, c, num18, positiveSign);
							flag = false;
						}
						else
						{
