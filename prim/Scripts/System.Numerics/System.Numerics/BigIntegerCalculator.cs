...
internal static class BigIntegerCalculator
{
	internal struct BitsBuffer
	{
		...
		{
			fixed (uint* bits2 = _bits)
			{
				fixed (uint* bits = value._bits)
				{
					fixed (uint* bits3 = temp._bits)
					{
						if (_length < value._length)
						{
							Multiply(bits, value._length, bits2, _length, bits3, _length + value._length);
						}
						else
						{
							Multiply(bits2, _length, bits, value._length, bits3, _length + value._length);
						}
					}
				}
			}
